...
class Config:
    ...
    LEARNING_RATE = 0.001       # Learning rate for client optimization
    TEST_RATIO = 0.2            # Ratio of test data per client
    SEED = 100                  # Random seed for reproducibility
    PLOT_STYLE = 'seaborn-v0_8' # Plot style
# ====================== Data Preparation ======================
class BatteryDataset(Dataset):
    def __init__(self, file_path, normalize_stats=True):
...
def main():
    ...
    for round in range(Config.ROUNDS):
        ...
        # Client training
        for client_idx in tqdm(selected, desc=f"Round {round+1}/{Config.ROUNDS}"):
        ...
        if avg_test_loss < best_loss:
            ...
            torch.save({
                ...
                'best_loss': best_loss,
                'round_losses': round_losses}, "adam_metrics.pth")        
        print(f"Round {round+1}: Test Loss = {avg_test_loss:.6f} | Best = {best_loss:.6f}")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"adam_fl_{round+1}.png", dpi=300)
    plt.close()  

    # Output  
    print("Training complete! Best model saved to 'adam_metrics.pth'")
